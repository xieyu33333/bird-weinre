...
module.exports = function () {
    this.transpond = function (req, res) {
        ...
        var transRules = require("../../config.js").TranspondRules;
        ...
        console.log(transCurrent.targetServer.host);
        var options = {
            host: transCurrent.targetServer.host,
            port: transCurrent.targetServer.port || 80,
        };
        options.headers = req.headers;
        options.headers.cookie = transCurrent.targetServer.headers.cookie;
        ...
        if (options.host.match(/http:\/\//)){
            console.error("***************************************************");
            console.error("****** error:  host should not contain http *******");
            console.error("***************************************************");
        }
        else{
